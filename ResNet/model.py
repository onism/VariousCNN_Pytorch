...
class BasicBlock(nn.Module):
    '''
        number of layers < 50
              3 * 3                   3*3
        x -> conv(x) -> bn -> relu -> conv(x) -> bn -> + ->
        |______________________________________________|
        if x is not equal with the output, should add a 1x1 conv to the input 
    '''
    expansion = 1 
...
class Bottleneck(nn.Module):
    '''
        number of layers > 50
             1 * 1                   3 * 3                   1*1
        x -> conv(x) -> bn ->relu -> conv(x) ->bn -> relu -> conv(x) -> bn -> + -> relu
        |_________________________________shortcut____________________________|
    '''
    expansion = 4
    def __init__(self, in_planes, planes, stride=1, use_batchnorm=True):
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        # x size: batch * channels * w * h
        out = F.relu(self.bn1(self.conv1(x)))
        # out size: batch * 64 * w * h
        out = self.layer1(out)
        # out size: batch * 64 * w * h 
        out = self.layer2(out)
        # out size: batch * 128 * w/2 * h/2
        out = self.layer3(out)
        # out size: batch * 256 * w/4 * h/4
        out = self.layer4(out)
        # out size: batch * 512 * w/8 * h/8
...
resnet10 = ResNet(BasicBlock,[1,1,1,1])
out = resnet10(torch.from_numpy(x))
print(out.size())
